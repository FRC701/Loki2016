...
OI::OI() {
  ...
  coA.reset(new JoystickButton(driver.get(), kButtonA_ID));
  coA->WhenPressed(new ToggleLifter());

  coB.reset(new JoystickButton(driver.get(), kButtonB_ID));
  coB->WhenPressed(new Cancel()); //TODO cancel command

  coX.reset(new JoystickButton(driver.get(), kButtonX_ID));
  coX->WhenPressed(new FullIntake(1.0));

  coY.reset(new JoystickButton(driver.get(), kButtonY_ID));
  coY->WhenPressed(new PrepShooter(1.0));

  coLB.reset(new JoystickButton(driver.get(), kButtonLB_ID));
  coLB->WhenPressed(new ToggleArms());

  coRB.reset(new JoystickButton(driver.get(), kButtonRB_ID));
  coRB->WhenPressed(new RollerOn(1.0));

  coBack.reset(new JoystickButton(driver.get(), kButtonBack_ID));
  coBack->WhenPressed(new RollerOn(1.0));

  coStart.reset(new JoystickButton(driver.get(), kButtonStart_ID));
  coStart->WhenPressed(new AutonomousCommand());

  coL3.reset(new JoystickButton(driver.get(), kButtonL3_ID));
  coL3->WhenPressed(new ToggleBrake());

  coR3.reset(new JoystickButton(driver.get(), kButtonR3_ID));
